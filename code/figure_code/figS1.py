...
for i, ax in enumerate(axs.flat):
    ...
    # 读取数据
    datanc = xr.open_dataset(f'/tera11/zhwei/students/Baifan/hard/PLSR/forcing/process_all/inter/std_mean/{names[i]}.nc')
...
x_values = range(1, 17) 
labels = [str(year) if year % 2 == 1 else '' for year in range(1995, 2011)]  
...
for i in range(1, 8):
    # 读取 NetCDF 文件
    file_path = f'/tera11/zhwei/students/Baifan/hard/PLSR/forcing/process_all/inter/std/fldmean/{names[i-1]}.nc'
...
ax.legend(loc='best', fontsize=31, ncol=7, frameon=False, bbox_to_anchor=(0.18, 0.77))
...
plt.savefig('/stu01/baif/job/test/S1.jpg', format='jpg', dpi=300, bbox_inches='tight')
input_file  = '/stu01/baif/job/test/S1.jpg'
output_file = '/stu01/baif/job/test/S1.jpg'
