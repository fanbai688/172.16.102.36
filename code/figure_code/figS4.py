...
ticks = [
    ...
    np.arange(vmin[2]+1, vmax[2], 2),
    np.arange(vmin[3]+100, vmax[3], 100)
]
boundaries = [
    np.arange(vmin[0], vmax[0]+0.06, 0.06),
    np.arange(vmin[1], vmax[1]+0.06, 0.06),
    np.arange(vmin[2], vmax[2]+0.06, 0.06),
    np.arange(vmin[3], vmax[3]+4, 4)
]
...
for i, ax in enumerate(axs.flat):
    if i == 4:
        break  
    # 读取数据
    datanc = xr.open_dataset(f'/tera11/zhwei/students/Baifan/hard/PLSR/variable/process/std_25year/CRUJRA_{names[i]}.nc')
...
for i in range(1, 5):
    # 读取 NetCDF 文件
    file_path = f'/tera11/zhwei/students/Baifan/hard/PLSR/variable/process/std_global/CRUJRA_{names[i-1]}.nc'
...
ax.legend(loc='best', fontsize=50, ncol=4, frameon=False)
# , bbox_to_anchor=(0.265, 0.77
...
plt.savefig('/stu01/baif/job/test/S4.jpg', format='jpg', dpi=300, bbox_inches='tight')
input_file  = '/stu01/baif/job/test/S4.jpg'
output_file = '/stu01/baif/job/test/S4.jpg'
